...
from viewer import predict_and_show,show_3_images,label2color
...
train_dir = '/home/liushixin/AI/chendali/shengjunGao/linknet/IronData_Camvid_train_val/train'
val_dir = '/home/liushixin/AI/chendali/shengjunGao/linknet/IronData_Camvid_train_val/val'
test_dir = '/home/liushixin/AI/chendali/shengjunGao/linknet/IronData_Camvid_train_val/val'

gt_train_dir = '/home/liushixin/AI/chendali/shengjunGao/linknet/IronData_Camvid_train_val/trainannot'
gt_val_dir = '/home/liushixin/AI/chendali/shengjunGao/linknet/IronData_Camvid_train_val/valannot'
gt_test_dir = '/home/liushixin/AI/chendali/shengjunGao/linknet/IronData_Camvid_train_val/valannot'
...
gt_train_files = [gt_train_file for gt_train_file in gt_train_files             if gt_train_file.endswith('.png')]
...
gt_val_files = [gt_val_file for gt_val_file in gt_val_files                 if gt_val_file.endswith('.png')]
...
def val_step(sess, net, max_step, val_init,val_batch,epoch):
    val_loss_list = []
    # Validation step
    ...
    for step in tqdm(range(max_step)):
        try:
            val_loss, _ ,pred,label,img= sess.run(
                [net.val_loss, net.val_metrics_update,net.val_pred,net.val_labels,net.val_images]
            )
            plNum=int(batch_size * 2)-1
            if epoch ==plNum:
            
                pred = np.array(pred,dtype='uint8')
                img=np.array(img,dtype='float32')
                img[epoch]=img[epoch]+np.array([103.939, 116.779, 123.68], dtype=np.float32)
                img=np.array(img,dtype='uint8')
                label2color(pred[epoch])
                label2color(label[epoch])
                show_3_images(img[epoch], pred[epoch], label[epoch], (16, 4),os.path.join(save_dir,str(epoch)+'.jpg'))
...
train_steps = int(math.ceil(1600 / batch_size))
val_steps = int(math.ceil(400 / batch_size / 2))
...
with tf.name_scope('input_pipe_line'):
    with tf.device('/cpu:0'):
        ...
        print('train_batch'*10,train_batch)
        val_dataset = val_dataset.batch(int(batch_size * 2))
...
for epoch in range(epochs):
    ...
    val_loss_list, val_accuracy, val_mean_iou = val_step(
        sess, net, val_steps, val_init,val_batch,epoch)
